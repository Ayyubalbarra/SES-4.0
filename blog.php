...
$latest_sql = "SELECT id, title, content, category, image FROM blogs WHERE visible = 1";
if (!empty($search)) {
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            if ($popular_result && $popular_result->num_rows > 0) {
                while ($row = $popular_result->fetch_assoc()) {
                    echo '<a href="detail.php?id=' . $row['id'] . '" class="block bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">';
                    echo '<div>';
                    ...
                    echo '<div class="p-4">';
                    echo '<h3 class="text-lg font-bold text-gray-800 mb-2 hover:text-gray-600 transition-colors">' . htmlspecialchars($row['title']) . '</h3>';
